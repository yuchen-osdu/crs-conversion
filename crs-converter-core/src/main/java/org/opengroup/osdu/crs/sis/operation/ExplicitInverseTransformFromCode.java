...
public class ExplicitInverseTransformFromCode implements ICRSCoordinateOperation {
    ...
    public ExplicitInverseTransformFromCode(ISisCrs fromSisCrs, ISisCrs toSisCrs, ISingleTrf transform) {
    ...
    private void initializeExplicitTransform() throws MismatchedDimensionException, FactoryException, NoninvertibleTransformException {
        CoordinateOperation toDatumOperation = CRS.findOperation(datumOperation.getSourceCRS(), toCrs, null);
        ...
        MathTransform step2 = datumOperation.getMathTransform().inverse();
        MathTransform step3 = toDatumOperation.getMathTransform();
        explicitMathTransform = MathTransforms.concatenate(step2, step3);
        if (!longLatOrder) {
            ...
            explicitMathTransform = MathTransforms.concatenate(MathTransforms.linear(swapMatrix), explicitMathTransform);
        }
    }

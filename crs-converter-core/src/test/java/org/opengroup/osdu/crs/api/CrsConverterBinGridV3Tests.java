...
import org.opengroup.osdu.crs.BinGrid.ConvertBinGridTests;
...
public class CrsConverterBinGridV3Tests {
	...
	@Test
	public void convertBinGridWithCRS() {
		ConvertBinGridResponse convertBinGridResponse = new ConvertBinGridResponse();
		convertBinGridResponse = ConvertBinGridTests.createResponse(Input_Response_With_CRS);
		String persistableReferenceCrs = ConvertBinGridTests
				.createPersistenceReferenceCRS(PersistenceReferenceCRS_With_CRS);
		...
		lenient().when(crsConverterApi.convertBinGrid(Mockito.any(), Mockito.any())).thenReturn(convertBinGridResponse);
		assertEquals(new Double(0.0), convertBinGridResponse.getMaxMisLocation().getDI());
		assertEquals(new Double(0.38), convertBinGridResponse.getMaxMisLocation().getDJ());
	}

	@Test
	public void convertBinGridWithoutCRS() {
		ConvertBinGridResponse convertBinGridResponse = new ConvertBinGridResponse();
		convertBinGridResponse = ConvertBinGridTests.createResponse(Input_Response_Without_CRS);
		String persistableReferenceCrs = ConvertBinGridTests
				.createPersistenceReferenceCRS(PersistenceReferenceCRS_Without_CRS);
		convertBinGridResponse.getOutBinGrid().getABCDBinGridSpatialLocation().getAsIngestedcoordinates()
				.setPersistableReferenceCrs(persistableReferenceCrs);
		lenient().when(crsConverterApi.convertBinGrid(Mockito.any(), Mockito.any())).thenReturn(convertBinGridResponse);
		assertEquals(new Double(0.0), convertBinGridResponse.getMaxMisLocation().getDI());
		assertEquals(new Double(0.0), convertBinGridResponse.getMaxMisLocation().getDJ());
	}

	@Test
	public void convertBinGridInvalidCRS() {
	...
	@Test
	public void convertBinGridInvalidPayload() {

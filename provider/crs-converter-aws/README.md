# CRS Converter Service
The Coordinate Reference System Conversion service converts the coordinates from one CRS to another.
...
***
If you run into an issue with Lombok on compile add '-Djps.track.ap.dependencies=false' to File > Settings > Build, Execution, Deployment > Compiler > Shared build process VM options. See https://stackoverflow.com/questions/65128763/java-you-arent-using-a-compiler-supported-by-lombok-so-lombok-will-not-work-a
***
...
```bash
mvn clean package -pl crs-converter-core,provider/crs-converter-aws
```

* If you wish to build the project without running tests

```bash
mvn clean package -pl crs-converter-core,provider/crs-converter-aws -DskipTests
```
...
<br/>
<br/>
NOTE: If not on osx/linux: Replace `*` with version numbers as defined in the provider/crs-converter-aws/pom.xml file

```bash
java -jar provider/crs-converter-aws/target/crs-converter-aws-*.*.*-SNAPSHOT-spring-boot.jar
```

## Testing

### Running Integration Tests
This section describes how to run OSDU Integration tests (testing/crs-converter-test-aws).
...
dist/testing/integration/bin/crs_converter_test_aws/build-aws/run-tests.sh 
```
